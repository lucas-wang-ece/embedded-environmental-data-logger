...
const char LOG_FILE_NAME[] = "sensor8.csv";      // CSV file name
...
void writeCsvHeaderIfNeeded() {
  if (!SD.exists(LOG_FILE_NAME)) {
    ...
    if (logFile) {
      ...
      Serial.println("Error: Could not create sensor8.csv");     //print error message
    }
  }
}
...
void enterIdleSleepMode() {
  set_sleep_mode(SLEEP_MODE_IDLE);    // Use idle sleep mode so Timer1 keeps running.

  sleep_enable();     // Enable sleep mode.

  sleep_mode();     //Set the CPU to sleep, the Timer1 interrupt will wake it up.
...
void logSensorData(unsigned long timeMs, float temperatureC, float humidity, bool alertOn) {     //access all the data
  ...
  if (logFile) {
    ...
    Serial.println("Error: Could not open sensor8.csv");
  }
}
...
void setup() {
  ...
  Serial.println("V8: Low-Power Sleep Mode Logging Started");
...
void loop() {
  if (sampleRequested) {          // Only read the sensor when the timer interrupt requests a sample.
    ...
    if (isnan(humidity) || isnan(temperatureC)) {       // Check whether either DHT11 sensor reading failed.
    ...
    enterIdleSleepMode();     //if no sample is currently requested, set the arduino into sleep mode
  }
