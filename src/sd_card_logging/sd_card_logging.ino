...
const char LOG_FILE_NAME[] = "sensor.csv";      // CSV file name
...
void writeCsvHeaderIfNeeded() {
  if (!SD.exists(LOG_FILE_NAME)) {
    ...
    if (logFile) {
      logFile.println("time_ms,temperature_c,humidity_percent,alert_status");       //print head message in the file, telling readers what each column represent
      logFile.close();      //close the file in order to store the data in the SD card
      Serial.println("CSV header written.");
    } else {
      Serial.println("Error: Could not create sensor.csv");     //print error message
    }
  }
}

void logSensorData(unsigned long timeMs, float temperatureC, float humidity, bool alertOn) {      //access time in MS, temperature, humidity, and alert
  File logFile = SD.open(LOG_FILE_NAME, FILE_WRITE);

  if (logFile) {          //if the file exist, print all the data with a "," as separation
    ...
    Serial.println("Error: Could not open sensor.csv");     //print error message
  }
}
